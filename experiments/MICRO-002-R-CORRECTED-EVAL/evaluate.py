...
SAVE_DIR = r'/home/bsierra/ltd-storage/results/canonical/MICRO-002-R-CORRECTED-EVAL'

MODEL_WEIGHTS = "ds3_best_multimodal_transformer_vec_3000.pth"
LABEL_ENCODER = "ds3_label_encoder_vec_3000.joblib"

MINMAX_SCALER_PATH = r"/home/bsierra/ltd-storage/artifacts/micro/MICRO-002-R/ds3_minmax_scaler_vec_3000.joblib"
_train_scaler = joblib.load(MINMAX_SCALER_PATH)
min_size_train = float(_train_scaler["min"])
max_size_train = float(_train_scaler["max"])
print(f"TRAIN scaler recuperado: min={min_size_train}, max={max_size_train}")
...
def load_dataset_full(csv_path):
    ...
    X_w_arr = np.array(X_w)
    X_w_norm = np.clip((X_w_arr - min_size_train) / (max_size_train - min_size_train + 1e-8), 0.0, 1.0)
