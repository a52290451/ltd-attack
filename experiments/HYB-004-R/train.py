...
=========================================================================================
📂 ARCHIVO: EXP3_Train_94F3000V_InclinadoF.py
🚀 VERSIÓN: 3.2 (Fusión Híbrida + Modality Dropout 50% Vectores + AMP + torch.compile)
👤 INVESTIGADOR: bsierra@zeus

-----------------------------------------------------------------------------------------
📝 RESUMEN ACADÉMICO / METODOLOGÍA:
    Entrenar el modelo Híbrido aplicando Modality Dropout (50%) a la rama de Vectores.
    Esto fuerza a la red a "inclinarse" hacia las 94 Features Invariantes, obligándola
    a extraer su máximo potencial estructural sin memorizar exclusivamente la forma
    dura de los paquetes.
...
SAVE_DIR = str(Path.home() / 'ltd-storage/results/canonical/HYB-004-R')
...
print("\n" + "═"*70)
print("🛡️ EXP 3 v3.2: ENTRENAMIENTO HÍBRIDO + MODALITY DROPOUT (50% VECTORES) + AMP + torch.compile")
print("═"*70)
...
joblib.dump(le, os.path.join(SAVE_DIR, "le_hibrido_incF.joblib"))
joblib.dump(scaler, os.path.join(SAVE_DIR, "scaler_hibrido_incF.joblib"))
...
# --- 5. ENTRENAMIENTO OPTIMIZADO (AMP bfloat16 + torch.compile + Gradient Clipping + Modality Dropout 50%) ---
...
print(f"\n🏋️ INICIANDO BUCLE DE ENTRENAMIENTO (MODALITY DROPOUT = 50% en Vectores + AMP bfloat16)")
for epoch in range(EPOCHS):
    ...
    for d, w, f, y_batch in train_loader:
        ...
        # ⚠️ MODALITY DROPOUT 50%: Apagamos la rama de VECTORES
        # La máscara se aplica ANTES de .to(device) para no generar tensores en GPU
        # y ANTES del autocast para que la operación sea determinista
        if np.random.rand() < 0.5:
            d_input = torch.zeros_like(d)
            w_input = torch.zeros_like(w)
        else:
            d_input = d
            w_input = w

        # Transferencia unificada a GPU con non_blocking
        d_input = d_input.to(device, non_blocking=True)
        w_input = w_input.to(device, non_blocking=True)
        f = f.to(device, non_blocking=True)
        y_batch = y_batch.to(device, non_blocking=True)

        with autocast("cuda", dtype=torch.bfloat16):
            loss = criterion(model(d_input, w_input, f), y_batch)
    ...
    if acc > best_acc:
        best_acc = acc
        best_epoch = epoch + 1
        torch.save(model.state_dict(), f"{SAVE_DIR}/best_model_hibrido_inclinadoF.pth")
        trigger = 0
    else:
        trigger += 1
        if trigger >= 15:
...
# --- 6. GENERACIÓN DE EVIDENCIA VISUAL (GRÁFICA 14) ---
print("\n🎨 Generando Gráfica 14: Dinámica de Convergencia (Inclinación a Features)...")
...
plt.subplot(1, 2, 1)
plt.plot(range(1, len(history_loss) + 1), history_loss, color='#8e44ad', linewidth=2.5, marker='s', markersize=4)
plt.axvline(best_epoch, color='gray', linestyle='--', alpha=0.7, label=f'Mejor Época ({best_epoch})')
plt.title('Efecto del Modality Dropout en el Error', fontsize=14, fontweight='bold', pad=10)
...
plt.subplot(1, 2, 2)
plt.plot(range(1, len(history_acc) + 1), history_acc, color='#2980b9', linewidth=2.5, marker='s', markersize=4)
plt.axvline(best_epoch, color='gray', linestyle='--', alpha=0.7, label=f'Mejor Época ({best_epoch})')
plt.title('Estabilización de Precisión (Dropout = 50% Vectores)', fontsize=14, fontweight='bold', pad=10)
plt.xlabel('Época de Entrenamiento', fontsize=12)
plt.ylabel('Precisión (%)', fontsize=12)
plt.legend()

plt.suptitle('Dinámica de Convergencia: Híbrido Inclinado a Features (AMP v3.2)', fontsize=16, fontweight='black', y=1.05)
plt.tight_layout()

# Guardar la imagen
grafica_path = os.path.join(SAVE_DIR, 'G14_Convergencia_Hibrido_InclinadoF.png')
