...
=========================================================================================
📂 ARCHIVO: EXP4_Evaluar_94F3000V_InclinadoV.py
🚀 VERSIÓN: 3.1.2 (Autopsia del Drift sobre Híbrido Inclinado a Vectores)
👤 INVESTIGADOR: bsierra@zeus

-----------------------------------------------------------------------------------------
📝 RESUMEN ACADÉMICO / METODOLOGÍA:
    Este script somete al modelo Híbrido entrenado con Modality Dropout (30% en Features)
    a la prueba de Concept Drift. 
    
    El objetivo es evaluar si "obligar" a la red a enfocarse en la secuencia cruda 
    (Vectores) durante el entrenamiento le otorgó mayor o menor resiliencia estructural
    frente al envejecimiento temporal, en comparación con el modelo neutro y el 
    inclinado a features.
    ...
    - Evidencia Visual: Generación de la Matriz de Confusión Post-Drift (Gráfica 19).
=========================================================================================
"""
import matplotlib
...
VECTORS_DRIFT_CSV = str(Path.home() / 'ltd-storage/datasets/future/CLEAN_final_vectors_sites_concept_drift.csv')
FEATURES_DRIFT_CSV = str(Path.home() / 'ltd-storage/datasets/future/CLEAN_final_features_sites_concept_drift.csv')

SAVE_DIR = str(Path.home() / 'ltd-storage/results/canonical/HYB-005-R')

timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
sys.stdout = Logger(os.path.join(SAVE_DIR, f"log_eval_EXP4_InclinadoV_{timestamp}.txt"))

print("\n" + "═"*70)
print("🛡️ EXP 4: EVALUACIÓN CONCEPT DRIFT (HÍBRIDO INCLINADO A VECTORES)")
print("═"*70)
...
try:
    le_entrenamiento = joblib.load(os.path.join(SAVE_DIR, "le_hibrido_incV.joblib"))
    scaler = joblib.load(os.path.join(SAVE_DIR, "scaler_hibrido_incV.joblib"))
...
print("🔌 Despertando red híbrida (Inclinada a Vectores)...")
model = MegaHybridModel(len(sitios_elite_entrenados), input_dim_features).to(device)
try:
    raw_state = torch.load(os.path.join(SAVE_DIR, "best_model_hibrido_inclinado_vec.pth"), map_location=device, weights_only=True)
...
print("\n" + "🏆"*20)
print(f"   ACCURACY FINAL (HÍBRIDO INCLINADO A VECTORES vs DRIFT): {acc*100:.2f}%")
print("🏆"*20 + "\n")

# --- 6. GENERACIÓN DE EVIDENCIA VISUAL (GRÁFICA 19) ---
print("🎨 Generando Gráfica 19: Matriz de Confusión del Concept Drift...")
...
sns.heatmap(cm_normalized, cmap="Oranges", cbar_kws={'label': 'Proporción de Predicciones'})

plt.title('Colapso de Fronteras: Matriz de Confusión Post-Drift\n(Modelo Inclinado a Vectores vs Futuro)', fontsize=15, fontweight='bold', pad=15)
...
plt.tight_layout()
grafica_path = os.path.join(SAVE_DIR, 'G19_Matriz_Drift_Hibrido_InclinadoV.png')
