"""
=========================================================================================
                    EXP4_Evaluar_Hibrido_Inclinado_Vectores_45F.py
=========================================================================================
"""
import pandas as pd
import numpy as np
...
SAVE_DIR = '../vectores_features/resultados_40F3000V_IncV'
...
try:
    ...
    le = joblib.load(os.path.join(SAVE_DIR, "le_45_inclinado_vec.joblib"))
    scaler = joblib.load(os.path.join(SAVE_DIR, "scaler_45_inclinado_vec.joblib"))
...
print("\n🔌 Despertando red híbrida inclinada a vectores...")
model = MegaHybridModel(len(memorized_ids), input_dim_features).to(device)
model.load_state_dict(torch.load(os.path.join(SAVE_DIR, "best_model_hibrido_inclinado_vec_45F.pth"), map_location=device))
model.eval()
...
print("\n" + "🏆"*20)
print(f"   ACCURACY FINAL (HÍBRIDO INCLINADO A VECTORES 45F vs DRIFT): {acc*100:.2f}%")
print("🏆"*20)