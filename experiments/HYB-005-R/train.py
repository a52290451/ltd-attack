...
=========================================================================================
📂 ARCHIVO: EXP4_Train_94F3000V_InclinadoV.py
🚀 VERSIÓN: 3.2 (Fusión Híbrida + Modality Dropout 30% en Features + AMP + torch.compile)
👤 INVESTIGADOR: bsierra@zeus

-----------------------------------------------------------------------------------------
📝 RESUMEN ACADÉMICO / METODOLOGÍA:
    Entrenar el modelo Híbrido aplicando Modality Dropout (30%) a la rama de FEATURES. 
    Esto fuerza a la red a "inclinarse" hacia los Vectores, maximizando la extracción
    de la firma estructural dura de los paquetes y usando las 94 invariantes temporales 
    solo como mecanismo de apoyo/desempate.
...
SAVE_DIR = str(Path.home() / 'ltd-storage/results/canonical/HYB-005-R')
os.makedirs(SAVE_DIR, exist_ok=True)

timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
sys.stdout = Logger(os.path.join(SAVE_DIR, f"log_train_EXP4_{timestamp}.txt"))

print("\n" + "═"*70)
print("🛡️ EXP 4 v3.2: ENTRENAMIENTO HÍBRIDO CON MODALITY DROPOUT (30% FEATURES) + AMP + torch.compile")
print("═"*70)
...
joblib.dump(le, os.path.join(SAVE_DIR, "le_hibrido_incV.joblib"))
joblib.dump(scaler, os.path.join(SAVE_DIR, "scaler_hibrido_incV.joblib"))
...
print(f"\n🏋️ INICIANDO BUCLE DE ENTRENAMIENTO (MODALITY DROPOUT = 30% en Features + AMP bfloat16)")
for epoch in range(EPOCHS):
    ...
    for d, w, f, y_batch in train_loader:
        ...
        # ⚠️ MODALITY DROPOUT: Apagamos las FEATURES el 30% de las veces
        if np.random.rand() < 0.3:
            f_input = torch.zeros_like(f)
        else:
            f_input = f

        with autocast("cuda", dtype=torch.bfloat16):
            loss = criterion(model(d, w, f_input), y_batch)
    ...
    if acc > best_acc:
        best_acc = acc
        best_epoch = epoch + 1
        torch.save(model.state_dict(), f"{SAVE_DIR}/best_model_hibrido_inclinado_vec.pth")
        trigger = 0
    else:
        trigger += 1
        if trigger >= 15:
...
# --- 6. GENERACIÓN DE EVIDENCIA VISUAL (GRÁFICA 15) ---
print("\n🎨 Generando Gráfica 15: Dinámica de Convergencia (Inclinación a Vectores)...")
...
plt.subplot(1, 2, 1)
plt.plot(range(1, len(history_loss) + 1), history_loss, color='#e67e22', linewidth=2.5, marker='d', markersize=4)
...
plt.subplot(1, 2, 2)
plt.plot(range(1, len(history_acc) + 1), history_acc, color='#27ae60', linewidth=2.5, marker='d', markersize=4)
plt.axvline(best_epoch, color='gray', linestyle='--', alpha=0.7, label=f'Mejor Época ({best_epoch})')
plt.title('Estabilización de Precisión (Dropout = 30% Features)', fontsize=14, fontweight='bold', pad=10)
plt.xlabel('Época de Entrenamiento', fontsize=12)
plt.ylabel('Precisión (%)', fontsize=12)
plt.legend()

plt.suptitle('Dinámica de Convergencia: Híbrido Inclinado a Vectores (Cross-Entropy + AMP v3.2)', fontsize=16, fontweight='black', y=1.05)
plt.tight_layout()

# Guardar la imagen
grafica_path = os.path.join(SAVE_DIR, 'G15_Convergencia_Hibrido_InclinadoV.png')
