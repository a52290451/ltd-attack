...
=========================================================================================
📂 ARCHIVO: EXP2_Evaluar_94F3000V_Neutro.py
🚀 VERSIÓN: 3.1.2 (Autopsia del Drift con Fix Pandas y Auditoría de Alineación)
👤 INVESTIGADOR: bsierra@zeus

-----------------------------------------------------------------------------------------
📝 RESUMEN ACADÉMICO / METODOLOGÍA:
    Este script somete al modelo Híbrido Neutro (Vectores + 94 Invariantes) a una 
    prueba de estrés longitudinal inyectándole datos capturados en el futuro.

    [!] ACTUALIZACIÓN V3.1.2 (RIGOR DE ABLACIÓN):
    - Auditoría de Alineación Integrada: Se imprime en consola la forma de los tensores 
      y se inspecciona una muestra aleatoria para garantizar que el cruce temporal 
      entre Macro (24h) y Micro (Vectores) es matemáticamente perfecto.
=========================================================================================
"""
import matplotlib
...
SAVE_DIR = str(Path.home() / 'ltd-storage/results/canonical/HYB-003-R')

timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
sys.stdout = Logger(os.path.join(SAVE_DIR, f"log_eval_EXP2_Neutro_{timestamp}.txt"))

print("\n" + "═"*70)
print("🛡️ EXP 2: EVALUACIÓN CONCEPT DRIFT (HÍBRIDO NEUTRO + 94 INVARIANTES)")
print("═"*70)
...
try:
    le_entrenamiento = joblib.load(os.path.join(SAVE_DIR, "le_hibrido_neutro.joblib"))
...
if len(y_ts) > 0:
    ...
    print(f"   - Logitud Vector Dirección: {len(X_d_ts[sample_idx])} | Longitud Vector Tamaño: {len(X_w_ts[sample_idx])}")
...
print("🔌 Despertando red neuronal híbrida...")
model = MegaHybridModel(len(sitios_elite_entrenados), input_dim_features).to(device)
try:
    raw_state = torch.load(os.path.join(SAVE_DIR, "best_model_hibrido_neutro.pth"), map_location=device, weights_only=True)
...
print("\n" + "🏆"*20)
print(f"   ACCURACY FINAL (HÍBRIDO NEUTRO vs DRIFT): {acc*100:.2f}%")
print("🏆"*20 + "\n")

# --- 6. GENERACIÓN DE EVIDENCIA VISUAL (GRÁFICA 17) ---
print("🎨 Generando Gráfica 17: Matriz de Confusión del Concept Drift...")
...
sns.heatmap(cm_normalized, cmap="Reds", cbar_kws={'label': 'Proporción de Predicciones'})

plt.title('Colapso de Fronteras: Matriz de Confusión Post-Drift\n(Modelo Híbrido Neutro vs Futuro)', fontsize=15, fontweight='bold', pad=15)
...
plt.tight_layout()
grafica_path = os.path.join(SAVE_DIR, 'G17_Matriz_Drift_Hibrido_Neutro.png')
